...
// Staleness = how long the ball has been sitting with whoever needs to act.
// - Reviewers have the ball (pending/re_review_needed): clock from last push
// - Author has the ball (changes_requested): clock from last changes_requested review
// - Ready to merge: clock from last push
// Using updated_at would be wrong — it changes on any activity (comments, CI, other reviews).
export function getStaleness(pr) {
  const blockedBy = getBlockedBy(pr);
  return blockedBy === "author" ? pr.lastChangesRequestedHours : pr.lastPushHours;
}
